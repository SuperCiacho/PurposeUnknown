export class Validator<T, TKey extends keyof T = keyof T> {
    ...
    public register<THelperKey extends TKey>(
        property: THelperKey,
        ...validationRules: ValidationRule<T[THelperKey]>[]
    ): void {
...
type ValidationStore<T, TKey extends keyof T> = {
    [P in TKey]?: ValidationRule<T[TKey]>[];
};
type ValidationRule<T> = (value: T) => boolean;
type ValidationCallback = (isValid: boolean) => void;

export const Validations = {
    exists: <T>(value: T): boolean => !!value,
    minLength: (min: number) => (value?: string | ArrayLike<unknown>): boolean => !!value && value.length >= min,

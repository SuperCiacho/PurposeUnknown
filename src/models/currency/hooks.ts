...
export function useCurrencies(source?: string, area?: string, withSource?: boolean): Currency[] {
    ...
    React.useEffect(() => {
        if (!result && source) {
            trackPromise(
                service
                    .list(source, withSource)
                    .then(currencies => setData(store => ({ ...store, [source]: currencies }))),
                area
            );
        }
...
export function useExchange(source?: string, target?: string, area?: string): CurrencyExchange[] {
    ...
    React.useEffect(() => {
        if (!result && source && target) {
            trackPromise(
                service.getExchangeHistory(source, target).then(exchanges =>
                    setData(store => ({
                        ...store,
                        [source + target]: exchanges
                    }))
                ),
                area
            );
